...
def test_check_explizites_out_wird_nicht_geloescht():
    """--check mit explizitem --out loescht das Zielverzeichnis nicht."""
    with tempfile.TemporaryDirectory() as tmpdir:
        custom_out = Path(tmpdir) / "mein_ordner"
        custom_out.mkdir()
        assert custom_out.exists()
        Ergebnis = _run_check("--out", str(custom_out))
        assert custom_out.exists(), "Explizites Ausgabe-Verzeichnis darf nicht geloescht werden"


def test_modell_ohne_org_slash():
    """Modell-Namen ohne Org/Repo-Praefix werden sauber als models--<name> aufgeloest."""
    with tempfile.TemporaryDirectory() as tmpdir:
        fake_cache = Path(tmpdir) / "hub" / "models--custom-model"
        fake_cache.mkdir(parents=True)
        (fake_cache / "config.json").write_text('{}')

        env = {**os.environ, "HF_HUB_CACHE": str(Path(tmpdir) / "hub"),
               "MLX_OCR_MODEL": "custom-model"}
        Ergebnis = subprocess.run(
            [sys.executable, str(PDF2MD), "--check", "--fortschritt",
             "--out", str(Path(tmpdir) / "out")],
            cwd=str(REPO), env=env, capture_output=True, text=True, timeout=30,
